...
# Hiperparâmetros
# =========================
PCAP_IDLE_TIMEOUT = 1000.0  # segundos sem crescer -> encerra
PCAP_GROWTH_POLL = 1000.0  # checa a cada x segundos
...
# Colunas
# =========================
FEATURES = ["ip_proto", "ip_len", "ip_ttl", "ip_id", "ip_flags_df", "ip_flags_mf", "ip_frag_off", "src_port",
            "dst_port", "tcp_seq", "tcp_ack", "tcp_flags", "tcp_syn", "tcp_ack_flag", "tcp_rst", "tcp_fin", "tcp_psh",
            "tcp_urg", "tcp_ece", "tcp_cwr", "tcp_window", "tcp_dataofs", "tcp_payload_len", "tcp_mss", "tcp_wscale",
            "tcp_sackok", "tcp_tsval", "tcp_tsecr", "udp_len", "udp_checksum", "udp_payload_len", "iat_src",
            "src_pkts_1s", "src_bytes_1s", "label"]
FEATURE_DIM = 34
...
class _Metrics:
    ...
    def finalize_and_print(self, title="Avaliação Final por Arquivo (janela=5s)"):
        ...
        print(cm)
        print(
            f"[LATÊNCIA] média={lat.mean():.1f} ms | min={lat.min():.1f} ms | max={lat.max():.1f} ms | arquivos={len(lat)}")
...
ATTACK_IPS = {"192.168.5.11", "192.168.4.8", "192.168.5.2", "192.168.2.8",
              "192.168.5.10"}  # <-------------altere os endereços de ataque
...
def build_labels_from_ips(src_ip_arr: np.ndarray,
    ...
    is_attack = np.isin(src_ip_arr, np.array(list(attack_ips), dtype=object)) if attack_ips else np.zeros(
        src_ip_arr.shape, bool)
    is_benign = np.isin(src_ip_arr, np.array(list(benign_ips), dtype=object)) if benign_ips else np.zeros(
        src_ip_arr.shape, bool)
...
def stream_pcap_offline_and_yield_windows(pcap_path: str, window_seconds: float = WINDOW_SECONDS):
    ...
    while True:
        ...
        for line in proc.stdout:
            ...
            tcp_sport = _to_int(parts[9]);
            tcp_dport = _to_int(parts[10])
            udp_sport = _to_int(parts[11]);
            udp_dport = _to_int(parts[12])
...
def run_stream_offline_fast(model, scaler, train_medians, device, pcap_path: str):
    ...
    for win_idx, window_raw in enumerate(
            stream_pcap_offline_and_yield_windows(pcap_path, window_seconds=WINDOW_SECONDS)):
...
class DDoSDetector(nn.Module):
    def __init__(self, feature_dim=FEATURE_DIM, proj_dim=64, depth=2, heads=4, layer_dropout=0.1, use_scalenorm=True,
                 pre_norm=False):
        super().__init__()
...
def chunk_window(Xw_scaled: np.ndarray, seq_len: int, mode: str, stride: int | None) -> list[
...
# =========================
def evaluate_on_5s_windows(csv_path: str, scaler: MinMaxScaler, train_medians: pd.Series, model: nn.Module,
                           device: torch.device, window_seconds: float = 5.0):
    ...
    print(
        f"[INFO] Janelas totais: {len(grp)} | benignas: {int((grp['win_label_bin'] == 0).sum())} | ataque: {int((grp['win_label_bin'] == 1).sum())}")
...
def main():
    ...
    if MODE == "train":
        ...
        if TEST_WINDOWS_CSV:
            ...
            evaluate_on_5s_windows(TEST_WINDOWS_CSV, scaler, train_medians, model.to(device), device,
                                   window_seconds=5.0)
        else:
